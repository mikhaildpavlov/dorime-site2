const CONFIG = {
  contactEmail: "hello@dorime.studio",
  telegramUrl: "#",
  socialsUrl: "#",
  web3formsAccessKey: "d5dee1f5-c174-4c8a-99db-13ac68e37d8b"
};
...
const translations = {
  ...
  ru: {
    ...
    "contact.title":"Связаться с <em>DorimE</em>","contact.text":"Напишите нам или заполните форму. Мы ответим в течение одного рабочего дня и предложим следующий шаг.",
    "form.name":"Имя","form.namePlaceholder":"Как к вам обращаться?","form.company":"Компания","form.companyPlaceholder":"Название компании или проекта","form.contact":"Email / Telegram","form.contactPlaceholder":"Как с вами связаться?","form.message":"Расскажите о проекте","form.messagePlaceholder":"Что нужно озвучить, сроки, референсы","form.submit":"Отправить заявку","form.note":"Заявка будет отправлена напрямую в нашу студию.","footer.tag":"Студия озвучивания","footer.copy":"© 2026 DorimE. Все права защищены."
  }
};
...
function applyLanguage(lang){
  ...
  document.title = currentLang === "en" ? "DorimE — Voiceover Studio" : "DorimE — Студия озвучивания";
...
form?.addEventListener("submit", async e=>{
  ...
  data.set("subject", currentLang === "en" ? `New project — DorimE — ${name}` : `Новый проект — DorimE — ${name}`);
  data.set("from_name", "DorimE website");
  data.set("botcheck", "");
  data.set("replyto", contact);

  form.dataset.submitting="true";
  if(submitButton){submitButton.disabled=true;submitButton.textContent=currentLang === "en" ? "Sending…" : "Отправка…";}
  ...
  try {
    ...
    if(result.success){
      form.reset();
      alert(currentLang === "en" ? "Thank you! Your request has been sent." : "Спасибо! Ваша заявка отправлена.");
    } else {
